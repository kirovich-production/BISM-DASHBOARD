...
const SalesLabranzaMonthlyChart = forwardRef<ChartRef, SalesLabranzaMonthlyChartProps>(
  ({ monthlyData, quarterOptions, monthOptions, onFilterByQuarter, onFilterByComparison }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      exportToPNG: () => {
        if (chartRef.current) {
          ...
          link.download = `ventas-mensuales-labranza-${new Date().toISOString().split('T')[0]}.png`;
          link.href = url;
          link.click();
        }
      },
    ...
    const chartData = {
      ...
      datasets: [
        {
          label: 'Ventas Labranza',
          data: displayData.map(d => d.labranza),
...
SalesLabranzaMonthlyChart.displayName = 'SalesLabranzaMonthlyChart';

export default SalesLabranzaMonthlyChart;
