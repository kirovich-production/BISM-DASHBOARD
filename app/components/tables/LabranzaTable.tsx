      ...
      <div className="flex-1 overflow-auto">
        <div className="min-w-full inline-block align-middle">
          <table className="min-w-full divide-y divide-gray-300">
            {/* Header de la tabla */}
            <thead className="bg-green-600 sticky top-0 z-30">
              <tr>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data.categories.flatMap((category: EERRCategory, categoryIndex: number) => [
                ...
                <tr key={`cat-${categoryIndex}`} className="bg-gradient-to-r from-green-100 to-green-50 border-t-2 border-green-300">
                  <td className="sticky left-0 z-20 px-6 py-3 text-sm font-bold text-green-900 uppercase tracking-wide bg-gradient-to-r from-green-100 to-green-50 shadow-lg">
                    {category.name}
                  </td>
                  <td
                ...
                ...(category.total ? [
                  ...
                  >
                    <td className="sticky left-0 z-20 px-6 py-3 text-sm text-green-900 font-bold whitespace-nowrap border-r border-green-200 bg-green-50 shadow-lg">
                      {category.total.Item}
                    </td>
                    {/* Meses normales (solo Monto y %) */}
                    {regularMonths.map((month: string, monthIdx: number) => (
